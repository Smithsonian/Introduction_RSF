...
library(ctmm)
load("China_Course/Data/tapir.rda")
...
# Currently, rsf.fit only uses isotropic models
...
RSFS <- rsf.select(DATA,
                   AKDE,
                   R=R,
                   formula=~I(sqrt(tree))+tree+I(tree^2),integrator="Riemann",verbose=TRUE,trace=TRUE)
