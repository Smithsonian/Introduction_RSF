...
# Graph frequency distribution of each covariate.
nt.rsf.10 %>% 
...
# You may have many covariates, so doing this in a loop makes sense. First I'll making a vector of my variable names.
vars <- nt.rsf.10 %>% 
  select(anth_risk:woody_dist) %>% 
  names()

# Then, we can either use a for loop
...
# Or the map function to create a list of plots 
hist.plots <- map(vars, ~ nt.rsf.10 %>% 
                    select(case_, var = .x) %>% 
                    ggplot(aes(x = var,
                               after_stat(density),
                               col = case_)) + 
...
# Then put all the plots in one grid
cowplot::plot_grid(plotlist = hist.plots)

# Do the same using a boxplot
box.plots <- map(vars, ~
                   nt.rsf.10 %>% 
                   select(case_, var = .x) %>% 
                   ggplot(aes(y = var,
...
wb.sim %>% unnest(cols = result) %>% 
  ...
  labs(x = "Available points per Use locations)", 
       y = "Estimate") +
  theme_light()
```
