...
terra::plot(rsf.stack$fence_dist,
            range = c(0, 100),
            col = "black")
...
The goal here, however, is to contrast points where an animal was observed with locations that were potentially available.  To do so, we will create pseudo absence points generated throughout an animals' MCP home range.  The [amt](https://cran.r-project.org/web/packages/amt/index.html) function `random_points` is a convenient way to do this.

```{r Availability, message=FALSE, warning=FALSE, echo=TRUE}
# Here we are generating 10 times the number of "Use" points to get our list of "available" points. We'll start with this level to begin to inspect our data and look for initial patterns. Before running our final models we'll assess what a robust minimum # of available points should be.
nt.rsf.10 <- random_points(nt.trk.rs,
                      n = nrow(nt.trk.rs) * 10,
                      type ="regular") # Also could be "random"

# Where are these points located? 
# Within the 100% MCP home range.  That's the only option currently in the package.
# To generate points within a user defined polygon, we'd need to use st_sample().
...
# Assign weights to our available available points, making extremely high.
nt.rsf.10 <- nt.rsf.10 %>% 
...
## Extract Covariates
# Here we extract the value of each raster file for each tracking location and each random location. This is something you can do individually with spatial files using various commands either in sf or terra depending on the type of file. For rasters it is the terra::extract() function. This can be done with one line of code when we have our rasters in a stack. The extract_covariates() function in amt calls this terra::extract function.
